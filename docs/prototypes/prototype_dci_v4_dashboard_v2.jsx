import React, { useState, useMemo } from "react";

/* ============================================================
   Tableau de bord Contrôle Interne Groupe ERANOVE
   Bloc 3 — Prototype V4 : Dashboard consolidé (vue Directeur CI Groupe)
   12 filiales déclarantes × 7 indicateurs · Données fictives.
   ============================================================ */

/* ---------- Logos ERANOVE et 11 entités juridiques (base64) ---------- */
const LOGOS = {
  ERANOVE: "data:image/png;base64,",
  GS2E: "data:image/png;base64,",
  CIE: "data:image/png;base64,",
  SODECI: "data:image/png;base64,",
  KEKELI: "data:image/png;base64,",
  CIPREL: "data:image/png;base64,",
  ATINKOU: "data:image/png;base64,",
  ASOKH: "data:image/png;base64,",
  SDER: "data:image/png;base64,",
  OMILAYE: "data:image/png;base64,",
  AWALE: "data:image/png;base64,",
  SMART_ENERGY: "data:image/png;base64,",
};

/* ---------- Palette ERANOVE étendue ---------- */
const NAVY = "#16213A";
const NAVY_2 = "#243456";
const VERT = "#8DB93F";
const VERT_F = "#4F7B14";
const VERT_P = "#EDF5DE";
const BLEU = "#2E6DA4";
const BLEU_P = "#E3EFFA";
const AMBRE = "#D9902B";
const AMBRE_P = "#FCF1DE";
const TERRE = "#C15A38";
const TERRE_P = "#FBE9E2";
const GRIS = "#8A909B";
const GRIS_P = "#F1F2F4";
const OR = "#E8B93C";

const INDICATEURS = [
  { code:"PCI", n:1, court:"Plan CI", libelle:"Taux de mise en œuvre du Plan de Contrôle Interne", objectif:1.0 },
  { code:"CARTO", n:2, court:"Cartographies", libelle:"Taux de réalisation ou mise à jour des cartographies de risques", objectif:0.5 },
  { code:"AMR", n:3, court:"Actions maîtrise", libelle:"Taux de mise en œuvre des actions de maîtrise des risques niveaux 1 & 2", objectif:0.8 },
  { code:"QCI", n:4, court:"QCI", libelle:"Taux de réalisation des Questionnaires de Contrôle Interne planifiés", objectif:0.6 },
  { code:"TCI", n:5, court:"TCI", libelle:"Taux de réalisation des Tests de Contrôle Interne planifiés", objectif:1.0 },
  { code:"RECO_SEM", n:6, court:"Reco. séminaires", libelle:"Taux de mise en œuvre des recommandations issues des séminaires / journées CI", objectif:0.75 },
  { code:"RECO_CA", n:7, court:"Reco. comités d'audit", libelle:"Taux de mise en œuvre des recommandations issues des comités d'audit", objectif:1.0 },
];
const TOUS = INDICATEURS.map((i) => i.code);

/* 12 filiales déclarantes pour 11 entités juridiques — GS2E remonte par 2 sous-directions */
const FILIALES = [
  { code:"GS2E_SDCI", libelle:"GS2E — SDCI", ej:"GS2E", logo:"GS2E", corr:"K. Assamoi", affectes:TOUS,
    profil:{ perf:.72, debut:1, trous:[], n1:.63 } },
  { code:"GS2E_SDRM", libelle:"GS2E — SDRM", ej:"GS2E", logo:"GS2E", corr:"H. M.", affectes:["PCI","CARTO","AMR","RECO_SEM"],
    profil:{ perf:.70, debut:1, trous:[], n1:.60 } },
  { code:"CIE", libelle:"CIE", ej:"CIE", logo:"CIE", corr:"A. Koffi", affectes:TOUS,
    profil:{ perf:.93, debut:1, trous:[], n1:.86, sur:["RECO_SEM"] } },
  { code:"SODECI", libelle:"SODECI", ej:"SODECI", logo:"SODECI", corr:"M. Diomandé", affectes:TOUS,
    profil:{ perf:.84, debut:1, trous:[], n1:.79, creux:[4,5,6] } },
  { code:"KEKELI", libelle:"KEKELI", ej:"KEKELI", logo:"KEKELI", corr:"B. Ouattara", affectes:TOUS,
    profil:{ perf:.66, debut:1, trous:[], n1:.58, denZero:{ TCI:[1,2] } } },
  { code:"CIPREL", libelle:"CIPREL", ej:"CIPREL", logo:"CIPREL", corr:"S. N'Guessan", affectes:TOUS,
    profil:{ perf:.74, debut:1, trous:[], n1:.71 } },
  { code:"ATINKOU", libelle:"ATINKOU", ej:"ATINKOU", logo:"ATINKOU", corr:"R. Bamba", affectes:TOUS,
    profil:{ perf:.61, debut:3, trous:[], n1:.52 } },
  { code:"ASOKH", libelle:"ASOKH", ej:"ASOKH", logo:"ASOKH", corr:"L. Traoré", affectes:TOUS,
    profil:{ perf:.69, debut:1, trous:[5], n1:.64 } },
  { code:"SDER", libelle:"SDER", ej:"SDER", logo:"SDER", corr:"P. Yao", affectes:TOUS,
    profil:{ perf:.47, debut:1, trous:[], n1:.44, reperdu:"AMR" } },
  { code:"OMILAYE", libelle:"OMILAYE", ej:"OMILAYE", logo:"OMILAYE", corr:"F. Kouassi", affectes:TOUS,
    profil:{ perf:.44, debut:1, trous:[8], n1:.41, objSpec:{ PCI:0.8 } } },
  { code:"AWALE", libelle:"AWALE", ej:"AWALE", logo:"AWALE", corr:"D. Coulibaly", affectes:TOUS,
    profil:{ perf:.58, debut:4, trous:[], n1:null } },
  { code:"SMART_ENERGY", libelle:"SMART ENERGY", ej:"SMART ENERGY", logo:"SMART_ENERGY", corr:"N. Aké", affectes:TOUS,
    profil:{ perf:.88, debut:1, trous:[], n1:.81 } },
];

const MOIS = ["Janvier","Février","Mars","Avril","Mai","Juin","Juillet","Août","Septembre","Octobre","Novembre","Décembre"];
const DERNIER_MOIS = 8;                 // août 2026, période ouverte jusqu'au 10 septembre
const EXERCICE = 2026;                  // exercice courant
const EXERCICES = [2026, 2025];         // exercices ouverts dans le dispositif — 2024 n'existe pas (RG-28)
const derniersMoisDe = (a) => (a === EXERCICE ? DERNIER_MOIS : 12);
const AFFECTATIONS = FILIALES.reduce((a, f) => a + f.affectes.length, 0); // 81

/* ---------- Générateur déterministe, cumuls monotones (RG-46) ---------- */
function hash(s) { let h = 2166136261; for (let i = 0; i < s.length; i++) { h ^= s.charCodeAt(i); h = Math.imul(h, 16777619); } return (h >>> 0) / 4294967295; }

function point(fil, ind, mois, annee) {
  const p = fil.profil;
  const nMois = annee === EXERCICE ? DERNIER_MOIS : 12;
  const base = annee === EXERCICE ? p.perf : p.n1;
  if (base === null || base === undefined) return { etat: "SANS_HISTORIQUE" };
  if (annee === EXERCICE) {
    if (mois < p.debut) return { etat: "NON_SAISI" };
    if (p.trous?.includes(mois)) return { etat: "NON_SAISI" };
    if (p.denZero?.[ind.code]?.includes(mois)) return { etat: "NON_CALCULABLE" };
  }
  const jitter = (hash(fil.code + ind.code + annee) - 0.5) * 0.16;
  const cible = Math.max(0.12, Math.min(1.18, base + jitter));
  let t = cible * (0.34 + 0.66 * (mois / nMois));
  if (annee === EXERCICE && p.creux?.includes(mois)) t *= 0.72;
  if (annee === EXERCICE && p.sur?.includes(ind.code) && mois >= 6) t = Math.min(1.35, t * 1.32);
  if (annee === EXERCICE && p.reperdu === ind.code) {
    t = mois <= 5 ? Math.max(t, ind.objectif + 0.04) : ind.objectif - 0.13 - 0.01 * mois;
  }
  return { etat: "OK", taux: Math.max(0, t) };
}

/* L'objectif de référence est celui du référentiel groupe. Une dérogation décidée par le
   Directeur est portée par l'affectation : filiale × indicateur × exercice (RG-23, RG-24). */
const objectifDe = (fil, ind, annee) =>
  (annee === EXERCICE ? fil.profil.objSpec?.[ind.code] : undefined) ?? ind.objectif;
const pct = (t, d = 1) => (t * 100).toFixed(d).replace(".", ",") + " %";

/* ---------- Échelle chromatique : position par rapport à l'objectif ---------- */
function couleurs(taux, objectif) {
  if (taux == null) return { bg: GRIS_P, fg: GRIS, bd: "#E2E4E8" };
  if (taux > 1) return { bg: BLEU_P, fg: "#1B547F", bd: "#B4D3EC" };
  const r = objectif > 0 ? taux / objectif : 0;
  if (r >= 1) return { bg: VERT_P, fg: VERT_F, bd: "#CBE3A6" };
  if (r >= 0.85) return { bg: "#F6F7E6", fg: "#7A7A16", bd: "#E4E7BE" };
  if (r >= 0.6) return { bg: AMBRE_P, fg: "#96601A", bd: "#F0D6A4" };
  return { bg: TERRE_P, fg: "#98422A", bd: "#F0C7B8" };
}

/* ============================================================ */
export default function DashboardConsolideDCI() {
  const [annee, setAnnee] = useState(EXERCICE);
  const [moisBrut, setMois] = useState(DERNIER_MOIS);
  const [compareN1, setCompareN1] = useState(false);
  const [tri, setTri] = useState({ col: "score", desc: true });
  const [focus, setFocus] = useState("TOUS");
  const [survol, setSurvol] = useState(null);

  const nOuvert = derniersMoisDe(annee);
  const mois = Math.min(moisBrut, nOuvert);
  const anneeN1 = annee - 1;
  const n1Disponible = EXERCICES.includes(anneeN1);

  /* --- Calculs (RG-13, RG-17, RG-18, RG-19, RG-25, RG-26) --- */
  const lignes = useMemo(() => FILIALES.map((f) => {
    const cellules = INDICATEURS.map((ind) => {
      if (!f.affectes.includes(ind.code)) return { ind, etat: "NON_AFFECTE" };
      const p = point(f, ind, mois, annee);
      const n1 = n1Disponible ? point(f, ind, mois, anneeN1) : { etat: "SANS_HISTORIQUE" };
      const obj = objectifDe(f, ind, annee);
      const objGroupe = ind.objectif;
      const objN1 = n1Disponible ? objectifDe(f, ind, anneeN1) : null;
      return { ind, ...p, obj, objGroupe, objN1,
               derogatoire: obj !== objGroupe,
               objDifferentN1: n1.etat === "OK" && objN1 != null && objN1 !== obj,
               ecart: p.etat === "OK" && n1.etat === "OK" ? (p.taux - n1.taux) * 100 : null };
    });
    const calc = cellules.filter((c) => c.etat === "OK");
    const score = calc.length ? calc.reduce((a, c) => a + c.taux, 0) / calc.length : null;
    const attendus = f.affectes.length;
    const saisis = cellules.filter((c) => c.etat === "OK" || c.etat === "NON_CALCULABLE").length;
    const atteints = calc.filter((c) => c.taux >= c.obj).length;
    return { f, cellules, score, attendus, saisis, atteints, complet: saisis === attendus,
             vide: saisis === 0 };
  }), [mois, annee, anneeN1, n1Disponible]);

  const triees = useMemo(() => {
    const l = [...lignes];
    const val = (r) => tri.col === "score" ? (r.score ?? -1)
      : tri.col === "nom" ? r.f.libelle
      : tri.col === "remontee" ? r.saisis / r.attendus
      : (r.cellules.find((c) => c.ind.code === tri.col)?.taux ?? -1);
    l.sort((a, b) => {
      const va = val(a), vb = val(b);
      const c = typeof va === "string" ? va.localeCompare(vb) : va - vb;
      return tri.desc ? -c : c;
    });
    return l;
  }, [lignes, tri]);

  const scores = lignes.map((r) => r.score).filter((s) => s != null);
  const moyenneGroupe = scores.length ? scores.reduce((a, b) => a + b, 0) / scores.length : null;
  const remontees = lignes.reduce((a, r) => a + r.saisis, 0);
  const tauxRemontee = remontees / AFFECTATIONS;
  const filialesCompletes = lignes.filter((r) => r.complet).length;
  const enRetard = lignes.filter((r) => !r.complet).length;
  const toutAtteint = lignes.filter((r) => r.score != null && r.atteints === r.attendus).length;

  const moyenneParIndicateur = INDICATEURS.map((ind) => {
    const v = lignes.map((r) => r.cellules.find((c) => c.ind.code === ind.code))
                    .filter((c) => c && c.etat === "OK").map((c) => c.taux);
    return { ind, moy: v.length ? v.reduce((a, b) => a + b, 0) / v.length : null, n: v.length };
  });

  const colonnes = focus === "TOUS" ? INDICATEURS : INDICATEURS.filter((i) => i.code === focus);

  /* Dérogations présentes dans les colonnes affichées — alimente l'avertissement sous le tableau */
  const derogations = lignes.flatMap((r) =>
    r.cellules.filter((c) => c.derogatoire && colonnes.some((k) => k.code === c.ind.code))
              .map((c) => ({ filiale: r.f.libelle, ind: c.ind.court, groupe: c.objGroupe, applique: c.obj })));
  const trierPar = (col) => setTri((t) => t.col === col ? { col, desc: !t.desc } : { col, desc: true });

  return (
    <div style={{ background: "#EFF1F5", minHeight: "100vh", fontFamily: "Arial, Helvetica, sans-serif", color: NAVY }}>

      <div style={{ background: "#FDF3E2", borderBottom: "1px solid #F2D9A8", color: "#8A5A12",
                    fontSize: 12, padding: "7px 24px" }}>
        <strong>Environnement de démonstration</strong> — toutes les données affichées sont fictives.
        Aucune donnée réelle du groupe ERANOVE n'est hébergée.
      </div>

      {/* En-tête */}
      <header style={{ background: `linear-gradient(100deg, ${NAVY} 0%, ${NAVY_2} 62%, #2C4066 100%)`,
                       padding: "18px 24px 20px", color: "#FFF" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
          <div style={{ background: "#FFF", borderRadius: 7, padding: "7px 11px", display: "flex" }}>
            <img src={LOGOS.ERANOVE} alt="ERANOVE" style={{ height: 30 }} />
          </div>
          <div style={{ flex: 1, minWidth: 240 }}>
            <div style={{ fontSize: 19, fontWeight: 700, letterSpacing: -.2 }}>
              Tableau de bord Contrôle Interne Groupe
            </div>
            <div style={{ fontSize: 12.5, color: "#AEBBD4", marginTop: 3 }}>
              Vue consolidée · 12 filiales déclarantes · Exercice {annee}
            </div>
          </div>
          <div style={{ background: "rgba(255,255,255,.10)", border: "1px solid rgba(255,255,255,.22)",
                        borderRadius: 8, padding: "9px 15px", lineHeight: 1.4 }}>
            <div style={{ fontSize: 13, fontWeight: 700 }}>Directeur Contrôle Interne Groupe</div>
            <div style={{ fontSize: 11.5, color: "#AEBBD4" }}>
              Accès complet · 12 filiales
              <span style={{ marginLeft: 6, background: "rgba(232,185,60,.2)", color: OR,
                             border: `1px solid ${OR}55`, borderRadius: 4, padding: "1px 5px",
                             fontSize: 10, fontWeight: 700 }}>ADMIN</span>
            </div>
          </div>
        </div>

        {/* Cartouche KPI */}
        <div style={{ marginTop: 18, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(210px, 1fr))", gap: 12 }}>
          <KPI titre="Moyenne des filiales" valeur={moyenneGroupe == null ? "—" : pct(moyenneGroupe)}
               sous={`moyenne des ${scores.length} scores calculables`} accent={VERT} />
          <KPI titre="Taux de remontée" valeur={pct(tauxRemontee, 0)}
               sous={`${remontees} saisies sur ${AFFECTATIONS} attendues`} accent={BLEU}
               jauge={tauxRemontee} />
          <KPI titre="Filiales à jour" valeur={`${filialesCompletes} / 12`}
               sous={enRetard === 0 ? "aucun retard de saisie" : `${enRetard} filiale(s) en retard`}
               accent={enRetard === 0 ? VERT : AMBRE} />
          <KPI titre="Tous objectifs atteints" valeur={`${toutAtteint} / 12`}
               sous={`au mois de ${MOIS[mois - 1].toLowerCase()}`} accent={OR} />
        </div>
      </header>

      {/* Onglets */}
      <nav style={{ background: "#FFF", borderBottom: "1px solid #E1E5EB", padding: "0 24px",
                    display: "flex", gap: 2, overflowX: "auto" }}>
        {["Dashboard consolidé","Détail par filiale","Détail par indicateur","Suivi des remontées","Paramétrage","Journal d'audit"]
          .map((t, i) => (
          <div key={t} style={{ padding: "12px 15px", fontSize: 13, whiteSpace: "nowrap",
            fontWeight: i === 0 ? 700 : 400, color: i === 0 ? NAVY : "#8A909B",
            borderBottom: i === 0 ? `2.5px solid ${VERT}` : "2.5px solid transparent" }}>{t}</div>
        ))}
      </nav>

      <main style={{ maxWidth: 1420, margin: "0 auto", padding: "20px 24px 60px" }}>

        {/* Barre de filtres */}
        <section style={{ background: "#FFF", border: "1px solid #E1E5EB", borderRadius: 10,
                          padding: "13px 18px", display: "flex", gap: 18, alignItems: "flex-end", flexWrap: "wrap" }}>
          <Champ label="Exercice">
            <select value={annee}
                    onChange={(e) => { const a = Number(e.target.value); setAnnee(a); setMois(derniersMoisDe(a)); }}
                    style={{ ...selectStyle, minWidth: 130, fontWeight: 700 }}>
              {EXERCICES.map((a) => (
                <option key={a} value={a}>{a}{a === EXERCICE ? " — en cours" : " — clos"}</option>
              ))}
            </select>
          </Champ>
          <Champ label="Mois de reporting">
            <select value={mois} onChange={(e) => setMois(Number(e.target.value))} style={selectStyle}>
              {MOIS.map((m, i) => (
                <option key={m} value={i + 1} disabled={i + 1 > nOuvert}>
                  {m} {annee}{i + 1 > nOuvert ? " — non ouvert" : ""}
                </option>
              ))}
            </select>
          </Champ>
          <Champ label="Focus indicateur">
            <select value={focus} onChange={(e) => setFocus(e.target.value)} style={{ ...selectStyle, minWidth: 210 }}>
              <option value="TOUS">Les 7 indicateurs</option>
              {INDICATEURS.map((i) => <option key={i.code} value={i.code}>{i.n}. {i.court}</option>)}
            </select>
          </Champ>
          <label title={n1Disponible ? "" : `Aucun exercice ${anneeN1} dans le dispositif`}
                 style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5,
                          cursor: n1Disponible ? "pointer" : "not-allowed", opacity: n1Disponible ? 1 : .55,
                          background: "#F6F8FA", border: "1px solid #E1E5EB", borderRadius: 7, padding: "9px 13px" }}>
            <input type="checkbox" disabled={!n1Disponible} checked={compareN1 && n1Disponible}
                   onChange={(e) => setCompareN1(e.target.checked)} />
            Écart vs {anneeN1}
          </label>
          <div style={{ flex: 1 }} />
          {annee === EXERCICE && mois === DERNIER_MOIS && (
            <div style={{ background: VERT_P, border: "1px solid #CBE3A6", color: VERT_F, borderRadius: 7,
                          padding: "8px 13px", fontSize: 12 }}>
              Période <strong>ouverte</strong> jusqu'au 10 septembre — les chiffres peuvent encore évoluer
            </div>
          )}
          {annee !== EXERCICE && (
            <div style={{ background: GRIS_P, border: "1px solid #DDE0E5", color: "#5B6473", borderRadius: 7,
                          padding: "8px 13px", fontSize: 12 }}>
              Exercice <strong>clos</strong> — données figées, lecture seule
            </div>
          )}
          <div style={{ display: "flex", gap: 8 }}>
            <Bouton>Export PDF</Bouton>
            <Bouton>Export Excel</Bouton>
          </div>
        </section>

        {/* Matrice consolidée */}
        <section style={{ marginTop: 16, background: "#FFF", border: "1px solid #E1E5EB", borderRadius: 10, overflow: "hidden" }}>
          <div style={{ padding: "14px 18px 12px", borderBottom: "1px solid #EDEFF2",
                        display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            <div style={{ width: 4, height: 20, background: VERT, borderRadius: 2 }} />
            <div style={{ fontSize: 15, fontWeight: 700 }}>
              Performance consolidée — {MOIS[mois - 1]} {annee}
            </div>
            <div style={{ fontSize: 11.5, color: GRIS }}>
              cumul depuis le 1<sup>er</sup> janvier · cliquer sur un en-tête pour trier
            </div>
          </div>

          <div style={{ overflowX: "auto" }}>
            <table style={{ borderCollapse: "separate", borderSpacing: 0, width: "100%", minWidth: 980 }}>
              <thead>
                <tr>
                  <Th onClick={() => trierPar("nom")} actif={tri.col === "nom"} desc={tri.desc}
                      style={{ textAlign: "left", minWidth: 190, position: "sticky", left: 0, background: "#FAFBFC", zIndex: 2 }}>
                    Filiale
                  </Th>
                  {colonnes.map((ind) => (
                    <Th key={ind.code} onClick={() => trierPar(ind.code)} actif={tri.col === ind.code} desc={tri.desc}
                        title={ind.libelle}>
                      <div style={{ fontSize: 10.5, color: GRIS, fontWeight: 400 }}>{ind.n}</div>
                      {ind.court}
                      <div style={{ fontSize: 10, color: GRIS, fontWeight: 400, marginTop: 2 }}>
                        obj. groupe {Math.round(ind.objectif * 100)} %
                      </div>
                    </Th>
                  ))}
                  <Th onClick={() => trierPar("remontee")} actif={tri.col === "remontee"} desc={tri.desc}>Remontée</Th>
                  <Th onClick={() => trierPar("score")} actif={tri.col === "score"} desc={tri.desc}
                      style={{ background: "#F2F5F9" }}>Score</Th>
                </tr>
              </thead>
              <tbody>
                {triees.map((r, rang) => (
                  <tr key={r.f.code} onMouseEnter={() => setSurvol(r.f.code)} onMouseLeave={() => setSurvol(null)}
                      style={{ background: survol === r.f.code ? "#F7F9FC" : "#FFF" }}>
                    <td style={{ ...tdStyle, position: "sticky", left: 0, zIndex: 1,
                                 background: survol === r.f.code ? "#F7F9FC" : "#FFF", borderRight: "1px solid #EDEFF2" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <span style={{ width: 20, fontSize: 11, color: rang < 3 ? OR : GRIS,
                                       fontWeight: rang < 3 ? 700 : 400 }}>{rang + 1}</span>
                        <img src={LOGOS[r.f.logo]} alt="" style={{ height: 20, maxWidth: 62, objectFit: "contain" }} />
                        <div>
                          <div style={{ fontSize: 12.5, fontWeight: 700 }}>{r.f.libelle}</div>
                          <div style={{ fontSize: 10.5, color: GRIS }}>{r.f.corr}</div>
                        </div>
                      </div>
                    </td>

                    {colonnes.map((ind) => {
                      const c = r.cellules.find((x) => x.ind.code === ind.code);
                      if (c.etat === "NON_AFFECTE") return (
                        <td key={ind.code} style={{ ...tdStyle, textAlign: "center", background: "#F7F8FA" }}
                            title="Indicateur non affecté à cette filiale — exclu de tous les calculs">
                          <span style={{ color: "#C3C8D1", fontSize: 15 }}>·</span>
                        </td>
                      );
                      if (c.etat === "NON_SAISI") return (
                        <td key={ind.code} style={{ ...tdStyle, textAlign: "center" }} title="Mois non saisi">
                          <span style={{ color: "#C3C8D1", fontSize: 12 }}>—</span>
                        </td>
                      );
                      if (c.etat === "SANS_HISTORIQUE") return (
                        <td key={ind.code} style={{ ...tdStyle, textAlign: "center" }}
                            title={`Aucun historique ${annee} pour cette filiale — entité intégrée au dispositif ultérieurement`}>
                          <span style={{ color: "#C3C8D1", fontSize: 11 }}>n.d.</span>
                        </td>
                      );
                      if (c.etat === "NON_CALCULABLE") return (
                        <td key={ind.code} style={{ ...tdStyle, textAlign: "center" }}
                            title="Dénominateur nul — exclu des moyennes, jamais compté comme 0 %">
                          <span style={{ color: GRIS, fontSize: 11.5, background: GRIS_P, border: "1px solid #E2E4E8",
                                         borderRadius: 4, padding: "2px 7px" }}>n/a</span>
                        </td>
                      );
                      const k = couleurs(c.taux, c.obj);
                      return (
                        <td key={ind.code} style={{ ...tdStyle, textAlign: "center" }}
                            title={`${r.f.libelle} · ${ind.libelle}\nTaux ${pct(c.taux)} — objectif appliqué ${pct(c.obj)}`
                                   + (c.derogatoire ? `\nOBJECTIF DÉROGATOIRE : le référentiel groupe fixe ${pct(c.objGroupe)}, le Directeur a retenu ${pct(c.obj)} pour l'exercice ${annee}.` : "")
                                   + (c.objDifferentN1 ? `\nL'objectif applicable en ${anneeN1} était de ${pct(c.objN1)} : l'écart compare deux niveaux d'exigence différents.` : "")}>
                          <div style={{ display: "inline-block", background: k.bg, color: k.fg, border: `1px solid ${k.bd}`,
                                        borderRadius: 5, padding: "4px 9px", minWidth: 56, fontWeight: 700, fontSize: 12.5,
                                        boxShadow: c.derogatoire ? `inset 0 0 0 1.5px ${AMBRE}` : "none" }}>
                            {pct(c.taux, 0)}
                            {c.derogatoire && <span style={{ marginLeft: 4, fontSize: 9, color: AMBRE }}>◆</span>}
                          </div>
                          {compareN1 && (
                            <div style={{ fontSize: 10, marginTop: 3,
                                          color: c.ecart == null ? GRIS : c.ecart >= 0 ? VERT_F : TERRE }}>
                              {c.ecart == null ? "n.d." : `${c.ecart >= 0 ? "+" : ""}${c.ecart.toFixed(0)} pts`}
                            </div>
                          )}
                        </td>
                      );
                    })}

                    <td style={{ ...tdStyle, textAlign: "center" }}>
                      <div style={{ fontSize: 12, fontWeight: 700, color: r.complet ? VERT_F : AMBRE }}>
                        {r.saisis} / {r.attendus}
                      </div>
                      <div style={{ height: 4, background: "#EDEFF2", borderRadius: 2, marginTop: 4, overflow: "hidden" }}>
                        <div style={{ width: `${(r.saisis / r.attendus) * 100}%`, height: "100%",
                                      background: r.complet ? VERT : AMBRE }} />
                      </div>
                    </td>

                    <td style={{ ...tdStyle, textAlign: "center", background: survol === r.f.code ? "#EEF3F9" : "#F8FAFC",
                                 borderLeft: "1px solid #EDEFF2" }}>
                      <span style={{ fontSize: 14.5, fontWeight: 700,
                        color: r.score == null ? GRIS : r.score >= .8 ? VERT_F : r.score >= .6 ? "#96601A" : TERRE }}>
                        {r.score == null ? "—" : pct(r.score, 0)}
                      </span>
                    </td>
                  </tr>
                ))}

                {/* Ligne de moyennes */}
                <tr style={{ background: "#F2F5F9" }}>
                  <td style={{ ...tdStyle, position: "sticky", left: 0, background: "#F2F5F9",
                               fontWeight: 700, fontSize: 12.5, borderRight: "1px solid #E1E5EB", borderTop: "2px solid #DDE3EB" }}>
                    Moyenne par indicateur
                  </td>
                  {colonnes.map((ind) => {
                    const m = moyenneParIndicateur.find((x) => x.ind.code === ind.code);
                    const k = m.moy == null ? null : couleurs(m.moy, ind.objectif);
                    return (
                      <td key={ind.code} style={{ ...tdStyle, textAlign: "center", borderTop: "2px solid #DDE3EB" }}
                          title={`Calculée sur ${m.n} filiales ayant une valeur calculable`}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: k ? k.fg : GRIS }}>
                          {m.moy == null ? "—" : pct(m.moy, 0)}
                        </div>
                        <div style={{ fontSize: 9.5, color: GRIS, marginTop: 2 }}>sur {m.n} filiales</div>
                      </td>
                    );
                  })}
                  <td style={{ ...tdStyle, textAlign: "center", borderTop: "2px solid #DDE3EB",
                               fontSize: 12.5, fontWeight: 700, color: BLEU }}>
                    {pct(tauxRemontee, 0)}
                  </td>
                  <td style={{ ...tdStyle, textAlign: "center", background: NAVY, color: "#FFF",
                               borderTop: "2px solid #DDE3EB" }}>
                    <span style={{ fontSize: 15, fontWeight: 700 }}>
                      {moyenneGroupe == null ? "—" : pct(moyenneGroupe, 0)}
                    </span>
                  </td>
                </tr>
              </tbody>
            </table>
          </div>

          {/* Légende */}
          <div style={{ borderTop: "1px solid #EDEFF2", padding: "12px 18px", display: "flex", gap: 20,
                        flexWrap: "wrap", fontSize: 11.5, color: "#5B6473", alignItems: "center" }}>
            <Puce c={couleurs(1, 1)} t="Objectif atteint" />
            <Puce c={couleurs(1.2, 1)} t="Surperformance, aucun plafond" />
            <Puce c={couleurs(.9, 1)} t="Proche de l'objectif" />
            <Puce c={couleurs(.7, 1)} t="Écart marqué" />
            <Puce c={couleurs(.4, 1)} t="Écart critique" />
            <span><strong>n/a</strong> dénominateur nul, exclu des moyennes</span>
            <span><strong>—</strong> mois non saisi</span>
            <span><strong style={{ color: "#C3C8D1" }}>·</strong> indicateur non affecté</span>
            <span><strong style={{ color: AMBRE }}>◆</strong> objectif dérogatoire fixé par le Directeur</span>
          </div>

          {/* Avertissement explicite sur les objectifs modifiés (arbitrage HM) */}
          {derogations.length > 0 && (
            <div style={{ borderTop: "1px solid #EDEFF2", background: AMBRE_P, padding: "11px 18px",
                          fontSize: 11.5, color: "#7A4E10", lineHeight: 1.6 }}>
              <strong>Objectifs dérogatoires en vigueur sur l'exercice {annee}.</strong> L'objectif indiqué en tête
              de colonne est celui du référentiel groupe. Les cellules marquées ◆ sont évaluées sur un objectif
              modifié par le Directeur CI Groupe :{" "}
              {derogations.map((d, i) => (
                <span key={i}>
                  {i > 0 ? " · " : ""}
                  <strong>{d.filiale} × {d.ind}</strong> {pct(d.groupe, 0)} → {pct(d.applique, 0)}
                </span>
              ))}
              . Le score de la filiale, la couleur des cellules et le classement sont calculés sur l'objectif
              appliqué, jamais sur l'objectif groupe.
            </div>
          )}
        </section>

        {/* Classement */}
        <section style={{ marginTop: 16, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(400px, 1fr))", gap: 16 }}>
          <Carte titre="Classement des filiales" accent={OR}
                 sous={`score au mois de ${MOIS[mois - 1].toLowerCase()}`}>
            {[...lignes].filter((r) => r.score != null).sort((a, b) => b.score - a.score).map((r, i) => (
              <div key={r.f.code} style={{ display: "flex", alignItems: "center", gap: 11, padding: "6px 0" }}>
                <span style={{ width: 18, fontSize: 11.5, fontWeight: 700, color: i < 3 ? OR : GRIS }}>{i + 1}</span>
                <span style={{ width: 128, fontSize: 12, fontWeight: i < 3 ? 700 : 400 }}>{r.f.libelle}</span>
                <div style={{ flex: 1, height: 13, background: "#F1F3F6", borderRadius: 3, overflow: "hidden", position: "relative" }}>
                  <div style={{ width: `${Math.min(100, r.score * 100)}%`, height: "100%",
                    background: r.score >= .8 ? VERT : r.score >= .6 ? AMBRE : TERRE, borderRadius: 3 }} />
                </div>
                <span style={{ width: 46, textAlign: "right", fontSize: 12, fontWeight: 700 }}>{pct(r.score, 0)}</span>
              </div>
            ))}
          </Carte>

          <Carte titre="Points d'attention" accent={TERRE} sous="à traiter en priorité">
            {lignes.filter((r) => !r.complet).map((r) => (
              <Alerte key={r.f.code} couleur={r.vide ? GRIS : AMBRE}
                titre={r.vide ? `${r.f.libelle} — aucune donnée sur l'exercice ${annee}`
                              : `${r.f.libelle} — remontée incomplète`}
                texte={r.vide
                  ? `Cette filiale n'a aucune remontée au titre de ${annee}. Elle est exclue de la moyenne groupe, jamais comptée comme 0 %.`
                  : `${r.saisis} indicateur(s) saisi(s) sur ${r.attendus} attendus pour ${MOIS[mois - 1].toLowerCase()}.`
                    + (annee === EXERCICE && mois === DERNIER_MOIS ? " La période est ouverte jusqu'au 10 septembre." : " La période est close.")} />
            ))}
            {lignes.filter((r) => r.score != null && r.score < 0.55).map((r) => (
              <Alerte key={r.f.code + "s"} couleur={TERRE} titre={`${r.f.libelle} — score critique`}
                texte={`Score de ${pct(r.score, 0)}, soit ${r.atteints} objectif(s) atteint(s) sur ${r.attendus}. Un échange avec le correspondant est recommandé.`} />
            ))}
            {lignes.flatMap((r) => r.cellules.filter((c) => c.etat === "OK" && c.taux > 1)
              .map((c) => (
                <Alerte key={r.f.code + c.ind.code} couleur={BLEU} titre={`${r.f.libelle} — ${c.ind.court} au-delà de 100 %`}
                  texte={`Taux de ${pct(c.taux)}. Aucun plafonnement n'est appliqué : vérifier la cohérence entre numérateur et dénominateur avec le correspondant.`} />
              )))}
          </Carte>
        </section>

        <footer style={{ marginTop: 24, fontSize: 11, color: GRIS, textAlign: "center", lineHeight: 1.7 }}>
          Prototype Bloc 3 · Écran V4 v2 — Dashboard consolidé · Règles RG-02, RG-03, RG-13, RG-17 à RG-19, RG-23 à RG-28<br />
          Groupe ERANOVE — Contrôle Interne Groupe · 12 filiales déclarantes, {AFFECTATIONS} affectations · Données fictives
        </footer>
      </main>
    </div>
  );
}

/* ---------- Sous-composants ---------- */
const selectStyle = { border: "1px solid #CFD5DE", borderRadius: 6, padding: "8px 10px",
                      fontSize: 13, minWidth: 160, background: "#FFF", color: NAVY, fontFamily: "inherit" };
const tdStyle = { padding: "9px 10px", borderBottom: "1px solid #F1F3F6", fontSize: 12.5, verticalAlign: "middle" };

function Th({ children, onClick, actif, desc, style, title }) {
  return (
    <th onClick={onClick} title={title}
      style={{ padding: "10px 10px", background: "#FAFBFC", borderBottom: "1px solid #E1E5EB",
               fontSize: 11.5, fontWeight: 700, color: actif ? NAVY : "#5B6473", cursor: "pointer",
               textAlign: "center", whiteSpace: "nowrap", userSelect: "none", ...style }}>
      {children}
      {actif && <span style={{ marginLeft: 4, color: VERT }}>{desc ? "▾" : "▴"}</span>}
    </th>
  );
}

function KPI({ titre, valeur, sous, accent, jauge }) {
  return (
    <div style={{ background: "rgba(255,255,255,.09)", border: "1px solid rgba(255,255,255,.16)",
                  borderLeft: `3px solid ${accent}`, borderRadius: 8, padding: "12px 15px" }}>
      <div style={{ fontSize: 10.5, textTransform: "uppercase", letterSpacing: .5,
                    color: "#AEBBD4", fontWeight: 700 }}>{titre}</div>
      <div style={{ fontSize: 27, fontWeight: 700, color: "#FFF", lineHeight: 1.15, marginTop: 5 }}>{valeur}</div>
      {jauge != null && (
        <div style={{ height: 4, background: "rgba(255,255,255,.16)", borderRadius: 2, margin: "7px 0 5px", overflow: "hidden" }}>
          <div style={{ width: `${jauge * 100}%`, height: "100%", background: accent }} />
        </div>
      )}
      <div style={{ fontSize: 10.5, color: "#9EACC7", marginTop: jauge != null ? 0 : 6 }}>{sous}</div>
    </div>
  );
}

function Champ({ label, children }) {
  return (
    <div>
      <label style={{ display: "block", fontSize: 10.5, textTransform: "uppercase", letterSpacing: .4,
                      color: "#69707E", fontWeight: 700, marginBottom: 5 }}>{label}</label>
      {children}
    </div>
  );
}

function Bouton({ children }) {
  return (
    <button style={{ border: "1px solid #CFD5DE", background: "#FFF", color: "#4A5261", borderRadius: 6,
                     padding: "9px 14px", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>{children}</button>
  );
}

function Carte({ titre, sous, accent, children }) {
  return (
    <div style={{ background: "#FFF", border: "1px solid #E1E5EB", borderRadius: 10, padding: "15px 18px 17px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
        <div style={{ width: 4, height: 18, background: accent, borderRadius: 2 }} />
        <div style={{ fontSize: 14.5, fontWeight: 700 }}>{titre}</div>
        <div style={{ fontSize: 11, color: GRIS }}>{sous}</div>
      </div>
      {children}
    </div>
  );
}

function Alerte({ couleur, titre, texte }) {
  return (
    <div style={{ borderLeft: `3px solid ${couleur}`, background: "#FAFBFC", borderRadius: "0 6px 6px 0",
                  padding: "9px 13px", marginBottom: 9 }}>
      <div style={{ fontSize: 12.5, fontWeight: 700, color: couleur }}>{titre}</div>
      <div style={{ fontSize: 11.5, color: "#5B6473", marginTop: 3, lineHeight: 1.5 }}>{texte}</div>
    </div>
  );
}

function Puce({ c, t }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
      <span style={{ width: 15, height: 15, borderRadius: 4, background: c.bg, border: `1px solid ${c.bd}` }} />
      {t}
    </span>
  );
}
